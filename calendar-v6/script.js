// EdTech Events Calendar V6 - Clean Implementation
class Calendar {
    constructor() {
        ...
        this.attendance = {};
        this.interests = {};
        this.eventRegistrations = {}; // Per-event colleague registrations
        ...
        footer.innerHTML = `
            <div class="collaboration-section">
                ...
                <div class="interested-section">
                    <h5>Interested (${(this.eventRegistrations[eventId] || []).length})</h5>
                    <div class="interested-list">
                        ${(this.eventRegistrations[eventId] || []).length === 0 ? '<div style="color: #9ca3af; font-style: italic;">No one registered for this event yet</div>' : 
                          (this.eventRegistrations[eventId] || []).map(colleague => `
                            <div class="interested-item" style="display: flex; align-items: center; gap: 0.5rem; padding: 0.25rem 0;">
                ...
                <div class="attendance-section" style="margin-top: 1rem;">
                    <h5>Actually Join (${attendees.length})</h5>
                    <div class="attendance-list">
                        ${attendees.length === 0 ? '<div style="color: #9ca3af; font-style: italic;">No attendees yet</div>' : 
                          attendees.map(attendee => `
                            <div class="attendee-item" style="display: flex; align-items: center; gap: 0.5rem; padding: 0.25rem 0;">
                                ...
                                <div>
                                    <div style="font-weight: 600; color: #374151;">${attendee.name}</div>
                                </div>
                            </div>
                          `).join('')}
                    </div>
                ...
                <div class="notes-section" style="margin-top: 1rem;">
                    <h5>Discussion Thread (${notes.length} posts)</h5>
    ...
    addLink(eventId) {
        ...
        const linkContent = isUrl ? `🔗 ${content}` : content;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('EdTech Events Calendar V6 initialized');
});
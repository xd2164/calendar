// EdTech Events Calendar V12 - Clean Implementation
class Calendar {
    constructor() {
        ...
        footer.innerHTML = `
            <div class="collaboration-section">
                ...
                <div class="attendance-section" style="padding: 1rem; background: #f0fdf4; border-radius: 0.5rem; border: 1px solid #10b981; margin-bottom: 1rem;">
                    ...
                    ${this.currentUser ? `
                        <button onclick="window.calendar.toggleAttendance(${eventId})" class="attendance-btn" style="background: #10b981; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer; margin-top: 0.5rem;">
                            ${attendees.find(u => u.id === this.currentUser.id) ? 'Leave Event' : 'Join Event'}
                        </button>
                    ` : `
                        <div style="margin-top: 0.5rem;">
                            <button onclick="window.calendar.registerAndJoin(${eventId})" style="background: #10b981; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer;">
                                Register & Join Event
                            </button>
                        </div>
                    `}
    ...
    registerAndJoin(eventId) {
        ...
        this.toggleAttendance(eventId);
        this.saveAttendanceData();
    }

    toggleAttendance(eventId) {
        if (!this.currentUser) {
            this.registerAndJoin(eventId);
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    window.calendar = new Calendar();
        console.log('EdTech Events Calendar V12 initialized');
});
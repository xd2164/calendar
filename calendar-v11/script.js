// EdTech Events Calendar V11 - Clean Implementation
class Calendar {
    constructor() {
        ...
        this.interests = {}; // Store users interested in each event
        this.attendance = {}; // Store users attending each event
        this.notes = {}; // Store notes for each event
        this.userNotes = {}; // Store notes by user ID
    ...
    showEventDetails(eventId) {
        console.log('showEventDetails called for event:', eventId);
        ...
        // Store current event ID for refresh functionality
    ...
    addCollaborationFeatures(eventId) {
        console.log('addCollaborationFeatures called for event:', eventId);
        const footer = document.getElementById('collaborationFooter');
        if (!footer) {
            console.error('collaborationFooter element not found');
            return;
        }
        footer.style.display = 'block';
        
        const interestedUsers = this.interests[eventId] || [];
        const attendingUsers = this.attendance[eventId] || [];
        
        // Get all notes for this event from all users
        const eventNotes = [];
        Object.keys(this.userNotes).forEach(userId => {
            const userNotes = this.userNotes[userId] || [];
        ...
        eventNotes.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
        ...
        footer.innerHTML = `
            <div class="collaboration-section">
                ...
                <div class="interested-section" style="padding: 1rem; background: #fef3c7; border-radius: 0.5rem; border: 1px solid #f59e0b; margin-bottom: 1rem;">
                    <h5 style="color: #92400e; font-size: 1rem; margin-bottom: 0.5rem;">Interested (${interestedUsers.length})</h5>
                    <div class="interested-list">
                        ${interestedUsers.length === 0 ? '<div style="color: #9ca3af; font-style: italic;">No one registered yet</div>' : 
                          interestedUsers.map(user => `
                            <div class="user-item" style="display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 0; border-bottom: 1px solid #fbbf24;">
                                <div class="avatar" style="width: 2rem; height: 2rem; border-radius: 50%; background: linear-gradient(135deg, #f59e0b, #d97706); display: flex; align-items: center; justify-content: center; color: white; font-size: 0.75rem; font-weight: 600;">${user.name.charAt(0)}</div>
                                <div>
                    ...
                    ${this.currentUser ? `
                        <button onclick="window.calendar.toggleInterest(${eventId})" class="interest-btn" style="background: #f59e0b; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer; margin-top: 0.5rem;" title="Click to show interest in this event">
                            ${interestedUsers.find(u => u.id === this.currentUser.id) ? 'Remove Interest' : 'Show Interest'}
                        </button>
                    ` : `
                        <div style="margin-top: 0.5rem;">
                            <button onclick="window.calendar.registerUser(${eventId}, () => window.calendar.toggleInterest(${eventId}))" style="background: #f59e0b; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer;">
                                Register & Show Interest
                            </button>
                        </div>
                    `}
                </div>
                ...
                <!-- Attending Section -->
                <div class="attending-section" style="padding: 1rem; background: #f0fdf4; border-radius: 0.5rem; border: 1px solid #10b981; margin-bottom: 1rem;">
                    <h5 style="color: #059669; font-size: 1rem; margin-bottom: 0.5rem;">Attending (${attendingUsers.length})</h5>
                    <div class="attending-list">
                        ${attendingUsers.length === 0 ? '<div style="color: #9ca3af; font-style: italic;">No one registered yet</div>' : 
                    ...
                    ${this.currentUser ? `
                        <button onclick="window.calendar.toggleAttendance(${eventId})" class="attendance-btn" style="background: #10b981; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer; margin-top: 0.5rem;" title="Click to confirm attendance at this event">
                            ${attendingUsers.find(u => u.id === this.currentUser.id) ? 'Leave Event' : 'Confirm Attendance'}
                        </button>
                    ` : `
                        <div style="margin-top: 0.5rem;">
                            <button onclick="window.calendar.registerUser(${eventId}, () => window.calendar.toggleAttendance(${eventId}))" style="background: #10b981; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer;">
                ...
                <!-- Notes / Collaboration Area -->
                <div class="notes-section" style="padding: 1rem; background: #f8fafc; border-radius: 0.5rem; border: 1px solid #e2e8f0; margin-bottom: 1rem;">
                    <h5 style="color: #475569; font-size: 1rem; margin-bottom: 0.5rem;">Notes / Collaboration Area</h5>
                    
                    ${this.currentUser ? `
                        <div class="add-note-section" style="margin-bottom: 1rem;">
                            <textarea id="newNote${eventId}" placeholder="Share your notes, takeaways, or questions about this event..." style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; resize: vertical; min-height: 80px;"></textarea>
                            <button onclick="window.calendar.addNote(${eventId})" style="background: #4f46e5; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer; margin-top: 0.5rem;" title="Add your notes or takeaways about this event">
                                Add Note
                            </button>
                        </div>
                    ` : `
                        <div style="margin-bottom: 1rem;">
                            <button onclick="window.calendar.registerUser(${eventId})" style="background: #4f46e5; color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer;">
                                Register to Add Notes
                            </button>
                        </div>
                    `}
                    ...
                    <div class="notes-list" style="max-height: 300px; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 0.375rem; padding: 0.5rem; background: #f9fafb;">
                        ${eventNotes.length === 0 ? '<div style="color: #9ca3af; font-style: italic; text-align: center; padding: 1rem;">No notes yet. Be the first to share!</div>' :
                          eventNotes.map(note => `
                            <div class="note-item" style="background: white; padding: 0.75rem; border-radius: 0.5rem; margin-bottom: 0.5rem; border-left: 3px solid #4f46e5; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
                                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
                                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                                        <div class="avatar" style="width: 1.5rem; height: 1.5rem; border-radius: 50%; background: linear-gradient(135deg, #4f46e5, #3730a3); display: flex; align-items: center; justify-content: center; color: white; font-size: 0.625rem; font-weight: 600;">${note.author.charAt(0)}</div>
                                        <div>
                                    ...
                                    ${this.currentUser && note.author === this.currentUser.name ? `
                                        <div style="display: flex; gap: 0.25rem;">
                                            <button onclick="window.calendar.editNote(${eventId}, '${note.id}')" style="background: #3b82f6; color: white; border: none; padding: 0.25rem 0.5rem; border-radius: 0.25rem; cursor: pointer; font-size: 0.75rem;" title="Edit your note">Edit</button>
                                            <button onclick="window.calendar.deleteNote(${eventId}, '${note.id}')" style="background: #ef4444; color: white; border: none; padding: 0.25rem 0.5rem; border-radius: 0.25rem; cursor: pointer; font-size: 0.75rem;" title="Delete your note">Delete</button>
                                        </div>
                                    ` : ''}
                                </div>
    ...
    // Collaboration Methods
    registerUser(eventId, callback = null) {
        console.log('registerUser called with eventId:', eventId);
        ...
        // Execute callback if provided (for actions that triggered registration)
    ...
    toggleInterest(eventId) {
        if (!this.currentUser) {
            this.registerUser(eventId, () => this.toggleInterest(eventId));
            return;
        }
        ...
        const existingIndex = this.interests[eventId].findIndex(u => u.id === this.currentUser.id);
    ...
    toggleAttendance(eventId) {
        if (!this.currentUser) {
            this.registerUser(eventId, () => this.toggleAttendance(eventId));
            return;
        }
        ...
        const existingIndex = this.attendance[eventId].findIndex(u => u.id === this.currentUser.id);
    ...
    addNote(eventId) {
        if (!this.currentUser) {
            this.registerUser(eventId, () => this.addNote(eventId));
            return;
        }
        ...
        if (!this.userNotes[this.currentUser.id]) {
            this.userNotes[this.currentUser.id] = [];
        }
        
        this.userNotes[this.currentUser.id].push(note);
    ...
    editNote(eventId, noteId) {
        const userNotes = this.userNotes[this.currentUser.id] || [];
        const note = userNotes.find(n => n.id === noteId && n.eventId === eventId);
        if (!note || note.author !== this.currentUser?.name) return;
    ...
    deleteNote(eventId, noteId) {
        if (!this.currentUser) return;
        const userNotes = this.userNotes[this.currentUser.id] || [];
        const noteIndex = userNotes.findIndex(n => n.id === noteId && n.eventId === eventId && n.author === this.currentUser.name);
        if (noteIndex >= 0) {
    ...
    saveCollaborationData() {
        localStorage.setItem('calendar_interests', JSON.stringify(this.interests));
        localStorage.setItem('calendar_attendance', JSON.stringify(this.attendance));
        localStorage.setItem('calendar_userNotes', JSON.stringify(this.userNotes));
        localStorage.setItem('calendar_currentUser', JSON.stringify(this.currentUser));
    }

    loadCollaborationData() {
        this.interests = JSON.parse(localStorage.getItem('calendar_interests')) || {};
        this.attendance = JSON.parse(localStorage.getItem('calendar_attendance')) || {};
        this.userNotes = JSON.parse(localStorage.getItem('calendar_userNotes')) || {};
        this.currentUser = JSON.parse(localStorage.getItem('calendar_currentUser')) || null;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.calendar = new Calendar();
        console.log('EdTech Events Calendar V11 initialized');
});